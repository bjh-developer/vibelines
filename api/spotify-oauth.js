...
async function handleGetTracks(req, res) {
  ...
  try {
    ...
    // Try different authentication methods for NocodeAPI
    // Method 1: Access token as query parameter (current approach)
    const nocodeUrl1 = `https://v1.nocodeapi.com/vibelines/spotify/${NOCODE_API_KEY}/myLibrary?access_token=${session.accessToken}&type=tracks&perPage=${parseInt(limit)}&page=${Math.floor(parseInt(offset) / parseInt(limit)) + 1}`;
    
    console.log('📡 NocodeAPI Request (Method 1 - Query Parameter):');
    console.log('URL (with token redacted):', nocodeUrl1.replace(session.accessToken, '[REDACTED]'));
    console.log('Access token length:', session.accessToken ? session.accessToken.length : 0);
    console.log('Token expires at:', new Date(session.expiresAt).toISOString());
    
    let nocodeResponse = await fetch(nocodeUrl1, {
      method: 'GET',
      headers: {
    ...
    console.log('📨 NocodeAPI Response status (Method 1):', nocodeResponse.status);
    
    // If method 1 fails with 401, try method 2: Authorization header
    if (nocodeResponse.status === 401) {
      console.log('🔄 Method 1 failed, trying Method 2 (Authorization Header)...');
      
      const nocodeUrl2 = `https://v1.nocodeapi.com/vibelines/spotify/${NOCODE_API_KEY}/myLibrary?type=tracks&perPage=${parseInt(limit)}&page=${Math.floor(parseInt(offset) / parseInt(limit)) + 1}`;
      
      nocodeResponse = await fetch(nocodeUrl2, {
        method: 'GET',
        headers: {
      ...
      console.log('📨 NocodeAPI Response status (Method 2):', nocodeResponse.status);
    }
    
    // If still failing, try refreshing the token regardless of expiry
    if (nocodeResponse.status === 401) {
      console.log('🔄 Still 401, force refreshing token...');
      const refreshed = await refreshUserToken(session);
      if (refreshed) {
        console.log('✅ Token force-refreshed, retrying...');
        const nocodeUrl3 = `https://v1.nocodeapi.com/vibelines/spotify/${NOCODE_API_KEY}/myLibrary?access_token=${session.accessToken}&type=tracks&perPage=${parseInt(limit)}&page=${Math.floor(parseInt(offset) / parseInt(limit)) + 1}`;
        
        nocodeResponse = await fetch(nocodeUrl3, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json'
          }
        ...
        console.log('📨 NocodeAPI Response status (After Refresh):', nocodeResponse.status);
      }
    }
